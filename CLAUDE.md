...
Agente personal controlado desde Telegram. Escribes desde el móvil y Luxy
ejecuta la tarea en el PC o el portátil que esté encendido, usando Claude Code,
Codex CLI o APIs HTTP configurables.
...
| `gateway` | webhook, autorización, cola, API privada, mensajes de Telegram | ejecutar código local |
...
```
Telegram → Cloudflare Worker → Supabase → (polling) → agente local → CLI/API → Telegram
```
